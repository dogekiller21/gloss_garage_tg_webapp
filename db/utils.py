...
async def update_service_category_price(service_id: int, category_id: int, default_price: int):
...
async def update_service_from_dict(
    ...
    updated_service = await update_from_dict(model=Service,
                                             pydantic_model=None,
    ...
    services_to_delete = [item for item in service_prices if (await item.category).id not in included_categories]
