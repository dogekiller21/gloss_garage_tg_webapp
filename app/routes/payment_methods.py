...
from db.pydantic_models import (
    PaymentMethod_pydantic, PaymentMethodCropped_pydantic, PaymentMethodIn_pydantic,
)
...
async def get_payment_methods(exclude_services: bool = True):
    parse_pydantic_model = PaymentMethodCropped_pydantic if exclude_services else PaymentMethod_pydantic
    methods = [await parse_pydantic_model.from_tortoise_orm(method)
               for method in (await PaymentMethod.all())]
    return methods


@router.get("/{payment_method_id:int}")
async def get_payment_method(payment_method: PaymentMethod = Depends(get_payment_method_depend)):
...
async def add_payment_method(payment_method_form: PaymentMethodIn_pydantic):
    ...
    return {**(await PaymentMethodCropped_pydantic.from_tortoise_orm(payment_method)).dict()}


@router.delete("/{payment_method_id:int}")
async def delete_payment_method(payment_method: PaymentMethod = Depends(get_payment_method_depend)):
